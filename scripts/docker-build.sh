...
if [ -n "${DOCKER_BUILD_ARGS}" ]; then
  ...
  export DOCKER_BUILD_ARGS="--pull --network=host --add-host albandrieu.com:172.17.0.57 --build-arg ANSIBLE_VAULT_PASS=${ANSIBLE_VAULT_PASS} --build-arg CI_PIP_GITLABNABLA_TOKEN=${CI_PIP_GITLABNABLA_TOKEN}"
...
# ${ANSIBLE_GALAXY_CMD} install -r "${WORKING_DIR}/../requirements.yml" -p ./roles/ --ignore-errors
...
time docker build ${DOCKER_BUILD_ARGS} -f "${WORKING_DIR}/../${DOCKER_FILE}" --tag "${DOCKER_ORGANISATION}/${DOCKER_NAME}" --tag "${DOCKER_ORGANISATION}/${DOCKER_NAME}:${DOCKER_TAG}" "${WORKING_DIR}/../" | tee docker.log
RC=$?
if [ ${RC} -ne 0 ]; then
  echo ""
...
#echo -e "    docker tag ${DOCKER_ORGANISATION}/${DOCKER_NAME}:latest ${DOCKER_REGISTRY_HUB}/${DOCKER_ORGANISATION}/${DOCKER_NAME}:latest ${NC}"
echo -e "    docker push ${DOCKER_REGISTRY}${DOCKER_ORGANISATION}/${DOCKER_NAME} ${NC}"
